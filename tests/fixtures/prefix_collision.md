...
- F20 — inference-based; needs human confirmation of ordering.
...
- **Severity:** high
- **Explanation:** the target `.git/config` declares `core.fsmonitor`, which git will execute.
...
### F20: Inference on scheduler ordering

- **Severity:** low
- **Explanation:** scheduler order is not proven deterministic.
- **Reasoning:** ordering inferred from file mtimes only; evidence-label inference.
- **Evidence:** data/state:9 · probe: census · fingerprint: sha256:c0ffee12 · rerun: `python3 scripts/probes/census.py <target>`
- **Verification:** inference · trusted · source-only · counterexample: explicit lock → deterministic.
- **Failure scenario:** two schedules interleave and reorder writes.
- **Remediation:** add an explicit ordering lock. fault-test: interleave harness.
- **Residual confidence:** ordering unverified · smallest safe probe: trace in sandbox.
